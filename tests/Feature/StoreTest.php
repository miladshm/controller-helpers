...
class StoreTest extends \Miladshm\ControllerHelpers\Tests\TestCase
{
    ...
    {
        $res = $this->postJson('/testing', ['code' => $uuid = fake()->uuid, 'name' => fake()->text(), 'order' => 1, 'status' => 1, 'parent_id' => 1]);
    ...
    {
        $res = $this->post('/testing', ['code' => $uuid = fake()->uuid, 'name' => fake()->text(), 'order' => 1, 'status' => 1, 'parent_id' => 1]);
